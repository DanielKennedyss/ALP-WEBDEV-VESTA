                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
                    ...
                    <article class="product-card group" style="animation-delay: {{ $index * 0.08 }}s">
                        ...
                        <div class="relative w-full flex flex-col items-center text-center pt-2">
                            ...
                            <button onclick="toggleWishlist(event, {{ json_encode([
                                'id' => $product->id,
                                'name' => $product->name,
                                'price' => $product->price,
                                'image_path' => $product->image_path,
                                'category' => $product->category->name ?? ''
                            ]) }})" class="absolute right-2 bottom-1.5 p-2 text-gray-400 hover:text-red-500 transition-colors z-20 origin-center" id="wishlist-heart-{{ $product->id }}" aria-label="Add to Wishlist">
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (quickviewId) {
        ...
        // Clean URL
        const newUrl = window.location.protocol + "//" + window.location.host + window.location.pathname + (window.location.search.replace(/quickview=\d+&?/, '').replace(/\?$/, ''));
